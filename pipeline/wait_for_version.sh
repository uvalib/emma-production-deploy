...
while true; do
   ...
   if [ "$VERSION" = "$EXPECTED_VERSION" ]; then
      echo "Reported version: $VERSION; success!"
      exit 0
   fi

   echo "Reported version: $VERSION, still waiting for: $EXPECTED_VERSION"

   # check for timeout
   NOW_TIME=$(date +%s)
   if [ $NOW_TIME -ge $END_TIME ]; then
      echo "ERROR: timed out waiting for the correct version to be reported"
      exit 1
   fi
